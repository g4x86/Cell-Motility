...
VertexHandles SurfaceTopology::updateLocalSurface(VertexHandle child_vertex_handle, FacetHandle start_facet_handle)
{
	...
	if(intersecting_facet_handle != facet_handle_null)
	{
		...
		FacetHandle closest_facet_handle = facet_handle_null;
		...
		if(closest_facet_handle != facet_handle_null && closest_distance <= intersecting_distance)
		{
	...
	else
	{
		for(VertexHandleHandle vhh = polygon_vertices.begin(); vhh != polygon_vertices.end(); ++vhh)
		{
			...
			for(EdgeHandleHandle ehh = (*vhh)->edges.begin(); ehh != (*vhh)->edges.end(); ++ehh)
			{
				...
				// Prerequisite #2
				if(begin_vertex_handle == *vhh1 && (*ehh)->dual == edge_handle_null) neighboring_edge_flag = true;
			}
...
bool SurfaceTopology::addFacet(VertexHandle vh1, VertexHandle vh2, VertexHandle vh3)
{
	...
	for(size_t i = 0; i < 3; ++i)
	{
		...
		if(eligibility)
		{
			...
			{
				if((*ehh)->prev->vertex == vhs[j] && (*ehh)->dual != edge_handle_null)
				{
					eligibility = false;
					break;
				}
...
void SurfaceTopology::removeFacet(FacetHandle fh)
{
	...
	for(size_t i = 0; i < 3; ++i)
	{
		...
		if(facet_edges[i]->dual != edge_handle_null)
		{
			facet_edges[i]->dual->dual = edge_handle_null;
		}
...
VertexHandles SurfaceTopology::removeVertex(VertexHandle vh)
{
	...
	assert(vh->filament != filament_branch_handle_null);
	...
	vh->filament->setVertex(vertex_handle_null);
...
VertexHandles SurfaceTopology::getNeighboringVertexHandles(VertexHandle vertex_handle) const
{
	...
	for(EdgeHandles::reverse_iterator ehh = vertex_edges.rbegin(); ehh != vertex_edges.rend(); ++ehh)
	{
		...
		if(next_dual == edge_handle_null) neighboring_vertices.push_back(next_edge->vertex);
...
bool SurfaceTopology::isBranchingAllowed(FilamentBranch& branch)
{
	...
	FacetHandle selected_branching_facet = facet_handle_null;
	Orientation selected_branching_orient;
	if(branch.getBranchingSiteActinConstHandle() != actin_handle_null)
	{
		ParameterTable::Table& param_table = ParameterTable::instance();
...
{
	FacetHandle intersecting_facet_handle = facet_handle_null;
